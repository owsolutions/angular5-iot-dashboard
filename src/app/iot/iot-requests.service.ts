...
export class IotRequestsService {
  ...
  public getLocations() {
    this.http.get(environment.api + '/api-iot/locations').subscribe(
      (response: any) => {
  ...
  public getUnconnected () {
    this.http.get(environment.api + '/api-iot/unconnected').subscribe(
      (response: any) => {
        const collections = response.data.items;
        for (const item of collections) {
          this.store.dispatch({
  ...
  public async getDevice (id: number) {
    try {
      const response: IResponse<CloudDevice> = await (this.http.get(environment.api + '/api-iot/device/' + id).toPromise());
      const collections = response.data.items;
      for (const item of collections) {
        this.store.dispatch({
          type: 'UPDATE_DEVICE',
  ...
  public getDevices () {
    this.http.get(environment.api + '/api-iot/devices').subscribe(
      (response: any) => {
        const collections = response.data.items;
        for (const item of collections) {
          this.store.dispatch({
  ...
  public async GetDeviceDayHistory(id: number, date: Date): Promise<IResponse<number>> {
    const url = environment.api + '/api-iot/devices/day-history/' + date + '/' + id;
  ...
  public async getDeviceDailyHisotry (id: number): Promise<IResponse<ICloudDeviceDailyHistory>> {
    const url = environment.api + '/api-iot/devices/daily-history/' + id;
    const ref = this.http.get(url).toPromise();
    try {
  ...
  public async PostDevice (device: CloudDevice): Promise<IResponse<CloudDevice>> {
    const ref = this.http.post(environment.api + '/api-iot/device' , device).toPromise();
    try {
  ...
  public async PostLocation (location: ILocation): Promise<IResponse<ILocation>> {
    const ref = this.http.post(environment.api + '/api-iot/location' , location).toPromise();
    try {
  ...
  async deleteLocation (id: number ) {
    const ref = this.http.delete(environment.api + '/api-iot/location/' + id).toPromise();
    try {
      const response: IResponse<ILocation> = await ref;
      if (IsSuccessEntity(response)) {
        const $location = response.data.items[0];
  ...
  async deleteDevice (id: number ) {
    const ref = this.http.delete(environment.api + '/api-iot/device/' + id).toPromise();
    try {
      const response: IResponse<ILocation> = await ref;
      if (IsSuccessEntity(response)) {
        const $location = response.data.items[0];
  ...
  async getDeviceToken (): Promise<IResponse<any>> {
    return await this.http.get(environment.api + '/api-iot/devices/token').toPromise();
  }
